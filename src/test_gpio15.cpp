...
void IRAM_ATTR sensorISR() {
  ...
  // Erhöhte Entprellzeit (50ms)
  static unsigned long lastTime = 0;
  if (currentTime - lastTime < 50000) {
    return;
  }
  
  // Zweite Überprüfung des Sensor-Status
  if (digitalRead(IR_SENSOR_PIN) != LOW) {
    return;  // Ignoriere falsche Auslösungen
  }
  ...
  if (lastPulseTime > 0) {
    ...
    // Ignoriere zu kurze Intervalle (Störungen)
    if (pulseInterval < 50000) {  // mind. 50ms (entspricht max. 600 RPM bei 2 Impulsen/Umdrehung)
      return;
    }
  }
...
void setup() {
  ...
  pinMode(IR_SENSOR_PIN, INPUT_PULLUP); // KEIN Pullup für diesen Sensor
